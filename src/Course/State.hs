...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s b
  (<$>) fab (State sta) =
    State $ \s -> let (a, t) = sta s in (fab a, t)
...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b
  (<*>) (State stsab) (State stsa) =
    State $ \s -> let (fab, s2) = stsab s1
                      (a, s1) = stsa s
                  in (fab a, s2)
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s b
  (=<<) f (State sa) =
    State $ \s -> let (a, s1) = sa s in runState (f a) s1
...
findM f (a :. as) = do
  found <- f a
  if found then pure (Full a) else findM f as
